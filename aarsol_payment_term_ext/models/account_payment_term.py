...
class AccountPaymentTermHoliday(models.Model):
    ...
    date_postponed = fields.Date('Postponed date', required=True)

    @api.constrains('holiday', 'date_postponed')
    def check_holiday(self):
        for rec in self:
            if fields.Date.from_string(rec.date_postponed) <= fields.Date.from_string(rec.holiday):
                raise ValidationError(_('Holiday %s can only be postponed into the future') % rec.holiday)
            if (rec.search_count([('payment_id', '=', rec.payment_id.id), ('holidayh', '=', rec.holiday),]) > 1):
                raise ValidationError(_('Holiday %s is duplicated in current payment term') % rec.holiday)
            if (rec.search_count([('payment_id', '=', rec.payment_id.id), '|', ('date_postponed', '=', rec.holiday), ('holiday', '=', rec.date_postponed)]) >= 1):
                raise ValidationError(_('Date %s cannot is both a holiday and a Postponed date') % rec.holiday)


class AccountPaymentTermLabel(models.Model):
    _name = 'account.payment.term.label'
    _description = 'Payment Term Label'

    sequence = fields.Integer('Sequence')
    name = fields.Char('Label')
    code = fields.Char('Code')
    type = fields.Selection([('admission','Admission'), ('main','Main'), ('installment','Installment'), ('add_drop','Add Drop'), ('other','Other')], 'Type', default='other', required=True)
    auto_publish = fields.Boolean('Auto Published', default=False)
...
class AccountPaymentTermLine(models.Model):
    ...
    option = fields.Selection(selection_add=[('custom', 'Custom Date')], ondelete={'custom': 'set default'}, default='day_current_month')
    amount_round = fields.Float('Amount Rounding', digits='Account', help='Sets the amount so that it is a multiple of this value.')
    ...
    payment_days = fields.Char(string='Payment day(s)',
        help='Put here the day or days when the partner makes the payment. Separate each possible payment day with dashes (-), commas (,) or spaces ( ).',)
    label_id = fields.Many2one('account.payment.term.label', 'Label')
    locked = fields.Boolean(related='payment_id.locked',store=True)
    ...
    def compute_line_amount(self, total_amount, remaining_amount, precision_digits):
        ...
        In case of procent computation, use the payment term line rounding if defined

            :param total_amount: total balance to pay
            :param remaining_amount: total amount minus sum of previous lines computed amount
            :returns: computed amount for this line
        """
        self.ensure_one()
        if self.value == 'fixed':
            return (-1 * float_round(self.value_amount, precision_digits=precision_digits))
...
class AccountPaymentTerm(models.Model):
    ...
    sequential_lines = fields.Boolean(default=False, help='Allows to apply a chronological order on lines.')
    holiday_ids = fields.One2many('account.payment.term.holiday','payment_id','Holidays')
    locked = fields.Boolean('Locked', default=False)
    portal = fields.Boolean('Available on Portal')
    ...
    def compute(self, value, date_ref=False, currency=None):
        ...
        self.ensure_one()
        last_account_move = self.env.context.get('last_account_move', False)   # Added this line
        ...
        for line in self.line_ids:
            ...
            if line.value == 'percent_amount_untaxed' and last_account_move:
                if last_account_move.company_id.currency_id == currency:
                    ...
                    raise UserError(_("Percentage of amount untaxed can't be used with foreign currencies"))
            ...
            if not self.sequential_lines:
                ...
                if float_is_zero(amt, precision_digits=precision_digits):
                    continue
            if line.option == 'custom':
                next_date = line.custom_date
            elif line.option == 'day_after_invoice_date':
                next_date += relativedelta(days=line.days, weeks=line.weeks, months=line.months)
                ...
                # Then add days
                next_date = next_first_date + relativedelta(days=line.days - 1, weeks=line.weeks, months=line.months)
            ...
            if not float_is_zero(amt, precision_digits=precision_digits):
                if amt < amount:
                    amt = amount
                result.append((fields.Date.to_string(next_date), amt, line.label_id.id))
                amount -= amt
