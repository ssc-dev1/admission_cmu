...
class DashboardStudentAdmission (Controller):
    ...
    def admission_student_dashboard(self, **kw):
        ...
        fee_invoice = request.env['account.move'].sudo().search(
            [('application_id', '=', application_id.id),('challan_type','=','admission')],limit=1,order='id desc')
        color_scheme = request.env['ir.config_parameter'].sudo(
        ).get_param('odoocms_admission_portal.color_scheme')
        ...
        return request.render('odoocms_admission_ext_ubas.admission_student_dashboard_ucp', context)
