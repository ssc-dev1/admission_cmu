...
class ScholarshipReceivedReport(models.TransientModel):
    _name = 'scholarship.received.report'
    _description = 'Student Scholarship Received Report'

    institute_ids = fields.Many2many('odoocms.institute', 'scholarship_received_rep_institute_rel', 'scholarship_received_rep_id', 'institute_id', 'Institutes')
    campus_ids = fields.Many2many('odoocms.campus', 'scholarship_received_rep_campus_rel', 'scholarship_received_rep_id', 'campus_id', 'Campuses')
    term_id = fields.Many2one('odoocms.academic.term', 'Term')

    def make_excel(self):
        workbook = xlwt.Workbook(encoding="utf-8")
        worksheet = workbook.add_sheet("Scholarship Received Report")
        ...
        worksheet.write_merge(0, 1, 0, 14, 'Scholarship Received Report Of ' + self.term_id.name, style=style_table_header2)
        row = 2
        col = 0
        table_header = ['Sr No.', 'CMS ID', 'Student NAME', 'School', 'Career', 'Scholarship Group', 'Scholarship NAME',
                        'Student Invoice', 'Student Invoice Status', 'Donor Invoice', 'Donor Invoice Status', 'Waiver', 'Hostel Invoice', 'Hostel Invoice Status', 'Total']
        for i in range(15):
        ...
        if fee_receipts:
            for fee_receipt in fee_receipts:
                ...
                col += 1
                worksheet.write(row, col, student_fee_receipts and student_fee_receipts[0].payment_state or '', style=style_date_col2)
                col += 1
                worksheet.write(row, col, fee_receipt.amount_total, style=style_date_col3)
                col += 1
                worksheet.write(row, col, fee_receipt and fee_receipt.payment_state or '', style=style_date_col2)
                col += 1
                ...
                col += 1
                worksheet.write(row, col, hostel_fee_receipts and hostel_fee_receipts[0].payment_state or '', style=style_date_col2)
                col += 1
        ...
        wiz_id = self.env['odoocms.fee.report.save.wizard'].create({
            ...
            'name': 'Scholarship Received Report.xls'
        })

        return {
            'type': 'ir.actions.act_window',
            'name': 'Scholarship Received Report',
