...
class OdoocmsShowChallanOnPortal(models.Model):
    ...
    faculty_type = fields.Selection([('Faculty', 'Select Faculty ...'), ('All', 'All Faculties'),],
        default='Faculty', tracking=True, index=True, string="Faculty Selection")
    program_selection_type = fields.Selection([('Program', 'Select Program ...'), ('All', 'All Program'),],
        default='Program', string='Program Selection', tracking=True, index=True)

    faculty_ids = fields.Many2many('odoocms.institute', 'show_challan_on_portal_institute_rel1', 'show_challan_on_portal_id', 'institute_id', 'Faculties')
    program_ids = fields.Many2many('odoocms.program', 'show_challan_on_portal_program_rel1', 'show_challan_on_portal_id', 'program_id', 'Programs')
    ...
    def action_confirm(self):
        ...
        if self.type in ('main_challan','2nd_challan'):
            if self.type == 'main_challan':
                installment = 1
            elif self.type == '2nd_challan':
                installment = 2
            domain = [('student_id', 'in', students.ids), ('term_id', '=', self.term_id.id), ('installment', '=', installment),('state','=','draft')]
            challans = self.env['odoocms.fee.barcode'].search(domain)
            challans.write({'show_on_portal': True})
    ...
    def action_hide_from_portal(self):
        ...
        for line in self.lines:
            ...
            if self.type in ('main_challan', '2nd_challan'):
                ...
                domain = [('student_id', 'in', students.ids), ('term_id', '=', self.term_id.id), ('installment', '=', installment), ('state', '=', 'draft')]
                challans = self.env['odoocms.fee.barcode'].search(domain)
                challans.write({'show_on_portal': False})

            else:
                domain = [('student_id', 'in', students.ids), ('term_id', '=', self.term_id.id), ('challan_type', '=', self.type),
                    ('payment_state', 'not in', ('in_payment', 'paid')), ('show_challan_on_portal', '=', True)]
                challans = self.env['account.move'].search(domain)
