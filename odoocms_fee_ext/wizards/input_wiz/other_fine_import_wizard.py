...
class OtherFineImportWizard(models.TransientModel):
    _name = "other.fine.import.wizard"
    _description = 'Other Fine Import'
    ...
    def action_other_fine_import(self):
        ...
        if self.import_option == 'csv':
            ...
            if self.file:
                ...
                other_fine_list = self.env['odoocms.input.other.fine']
                for row_no in range(sheet.nrows):
                    ...
                    if row_no <= 0:
                        ...
                        if student_id:
                            fine_type = line[1] and line[1].strip() or False
                            ...
                            if fine_type:
                                term_id = False
                                fine_type_id = self.env['odoocms.input.other.fine.type'].search([('name', '=', fine_type)])
                                if term_rec:
                                    term_id = self.env['odoocms.academic.term'].search([('name', '=', term_rec)])
                                if fine_type_id and term_id:
                                    values = {
                                        'student_id': student_id.id,
                                        'type': fine_type_id.id,
                                        'term_id': term_id and term_id.id or False,
                                        'amount': float(line[3]),
                                        'date': date_value,
                                    }
                                    new_rec = self.env['odoocms.input.other.fine'].create(values)
                                    other_fine_list |= new_rec
                if other_fine_list:
                    record_list = other_fine_list.mapped('id')
                    form_view = self.env.ref('odoocms_fee_ext.view_odoocms_input_other_fine_form')
                    tree_view = self.env.ref('odoocms_fee_ext.view_odoocms_input_other_fine_tree')
                    return {
                        'domain': [('id', 'in', record_list)],
                        'name': _('Other Fines'),
                        'view_type': 'form',
                        'view_mode': 'tree,form',
                        'res_model': 'odoocms.input.other.fine',
                        'view_id': False,
                        'views': [
