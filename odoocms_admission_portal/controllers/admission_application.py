...
class RegisterApplication(Controller):
    ...
    def get_application_data(self):
        ...
        application_id = request.env['odoocms.application'].sudo().search([('application_no', '=', current_user.login)])
        ...
        city_id = request.env['odoocms.city'].sudo().search([], order='name ASC')
        religion_id = request.env['odoocms.religion'].sudo().search([])
        profession_id = request.env['odoocms.profession'].sudo().search([])
        board_id = request.env['odoocms.application.board'].sudo().search(['|',('company_id', '=', request.env.user.company_id.id),('company_id','=',False)])
        passing_year = request.env['odoocms.application.passing.year'].sudo().search(['|',('company_id', '=', request.env.user.company_id.id),('company_id','=',False)])

        academic_id = request.env['applicant.academic.detail'].sudo().search([])
        undertaking = ''
        if application_id.register_id:
            undertaking = application_id.register_id.undertaking

        academic_data = request.env['odoocms.admission.education'].sudo().search(['|',('company_id' , '=' , request.env.user.company_id.id),('company_id','=',False)])
        ...
        last_institute_attend = request.env['last.institute.attend'].sudo().search(['|',('company_id', '=', request.env.user.company_id.id),('company_id','=',False)])
        advertisement = request.env['odoocms.advertisement'].sudo().search(['|',('company_id', '=', request.env.user.company_id.id),('company_id','=',False)])
        educational_institute_school = request.env['odoocms.application.institute.school'].sudo().search([])
        educational_institute_college = request.env['odoocms.application.institute.college'].sudo().search([])
        educational_institute_university = request.env['odoocms.application.institute.university'].sudo().search([])
        steps_id = request.env['odoocms.application.steps'].sudo().search([('company_id','=',company.id)], order='sequence')
        test_center_id  = request.env['test.center'].sudo().search(['|',('company_id', '=', request.env.user.company_id.id),('company_id','=',False)],order='sequence asc')
        ...
        academic_id = request.env['applicant.academic.detail'].sudo().search([('application_id', '=', application_id.id)])
        degree_intermediate = request.env['odoocms.degree'].sudo().search(['|',('company_id' , '=' , request.env.user.company_id.id),('company_id','=',False)])
        ...
        pgc_institute_id = request.env['pgc.institute'].sudo().search(['|',('company_id', '=', request.env.user.company_id.id),('company_id','=',False)])
        ...
        admit_card = request.env['applicant.entry.test'].sudo().search(
            [('student_id', "=", application_id.id), ('active', '=', True), ('paper_status', '=', False), ('slot_type', '=', 'test')])
        ...
        dob_minimum = request.env['ir.config_parameter'].sudo().get_param('odoocms_admission_portal.dob_min')
        dob_maximum = request.env['ir.config_parameter'].sudo().get_param('odoocms_admission_portal.dob_max')

        context = {
            'dob_minimum': datetime.strptime(dob_minimum, '%Y-%m-%d').date(),
            'dob_maximum': datetime.strptime(dob_maximum, '%Y-%m-%d').date(),
            ...
            'educational_institute_school': educational_institute_school,
            'educational_institute_college': educational_institute_college,
            'educational_institute_university': educational_institute_university,
            'last_institute_attend': last_institute_attend,
            ...
            'pgc_institute_id': pgc_institute_id,
    ...
    @route('/admission/application/', type='http', auth='user', csrf=False, website=True, methods=['GET', 'POST'])
    def admission_application(self, **kw):
        context = self.get_application_data()
            ...
            if request.httprequest.method == 'POST':
                ...
                if kw.get('step_name') == 'education':
                    ...
                    if kw.get('result_status') or alevel:
                        academic_data.update({
                            'result_status': 'complete' if alevel and not kw.get('result_status') else kw.get('result_status', '')
                        })
                ...
                if kw.get('step_name') == 'scholarship':
                    # Handle need-based scholarship checkbox
                    ...
                    if kw.get('pgc_institute'):
                        pgc.update({
                            'pgc_institute_id': int(kw.get('pgc_institute')) if kw.get('pgc_institute', '').isnumeric() else '',
    ...
    @route('/degree/specializations/', type='http', auth='user', methods=['POST'], csrf=False)
    def degree_specializations(self, **kw):
        try:
            degree_id = int(kw.get('degree_id'))
            specialization_domain = [('id', '=', degree_id), '|',('company_id', '=', request.env.user.company_id.id),('company_id','=',False),('active','=',True)]
            specilizations = request.env['odoocms.admission.degree'].sudo().search(specialization_domain).specialization_ids

            specialization_data = []
            for specilization in specilizations:
                specialization_data.append({
                    'id': specilization.id,
                    'name': specilization.name
                })
            record = {
                'status': "noerror",
    ...
    def degree_specializations_subject(self, **kw):
        ...
        try:
            ...
            specilizations_subject = request.env['applicant.academic.group'].sudo().search(
                [('id', '=', specialization_id),('company_id', '=', request.env.user.company_id.id),('active','=',True)]).academic_subject_ids
    ...
    def download_admission_slip(self, **kw):
        ...
        pdf_report = request.env.ref('odoocms_admission.action_report_application_fee_challan').sudo(
        )._render_qweb_pdf([application.id])[0]
    ...
    def download_fee_invoice(self, **kw):
        current_user = request.env.user
        # pdb.set_trace()
        application = request.env['odoocms.application'].sudo().search([('application_no', '=', current_user.login)])
        fee_invoice = request.env['odoocms.fee.barcode'].sudo().search(
            [('payment_id', '=', False), ('student_id', '=', application.student_id.id), ('label_id.type', '=', 'admission')], limit=1)
        if fee_invoice:
            pdf_report = request.env.ref('odoocms_fee_ext.action_report_admission_fee_challan').sudo(
            )._render_qweb_pdf([fee_invoice.id])[0]
            pdfhttpheaders = [('Content-Type', 'application/pdf'), ('Content-Length', len(pdf_report))]
            return request.make_response(pdf_report, headers=pdfhttpheaders)
    ...
    def apply_application(self, **kw):
        current_user = request.env.user
        application = request.env['odoocms.application'].sudo().search([('application_no', '=', current_user.login)])
        check_matric = application.applicant_academic_ids.filtered(lambda x: x.degree_name.year_age == 10)
        check_fsc = application.applicant_academic_ids.filtered(lambda x: x.degree_name.year_age == 12)
        ...
        if application.state == 'draft':
            ...
            template = request.env['mail.template'].sudo().search([('name', '=', 'Admission Application Form Submitted'), ('mail_server_id', '=', mail_server_id.id)])
            template.with_context(pass_val, email_from=email_from).send_mail(current_user.id, force_send=True)
    ...
    def delete_education(self, **kw):
        ...
        application_id.applicant_academic_ids.sudo().search([('id', '=', edu_id)]).unlink()
        preferences_allowed = 0
        # This whole if para was remarked by Ahsan
        if len(application_id.applicant_academic_ids) > 0:
            applicant_education_year = max([int(year.year_age) for year in application_id.applicant_academic_ids.degree_name])
            register_id = request.env['odoocms.admission.register'].sudo().search(
                [('state', '=', 'application'), ('min_edu_year', '<=', applicant_education_year)])

            if register_id:
                register_id_max = max([rec.min_edu_year for rec in register_id])
                register_id = register_id.filtered(lambda x: x.min_edu_year == register_id_max)
                application_id.register_id = register_id
                preferences_allowed = int(application_id.register_id.preferences_allowed)
    ...
    def prepare_admission_invoice(self, **kw):
        ...
        disciplines = 0
        discipline = 0
        prefs = request.env['odoocms.application.preference'].sudo().search([('application_id', '=', application_id.id)])
    ...
    def prepare_preference(self, **kw):
        try:
            ...
            specialization_edu = academic_info.filtered(lambda x: x.group_specialization)
            non_specialization_edu = academic_info.filtered(lambda x: not x.group_specialization)
    ...
    def download_attachment_file(self, model=None, id=0, **kw):
        ...
        status, headers, content = env['ir.http'].sudo()._binary_set_headers(status, content, filename, mimetype,
            unique=False, filehash=filehash, download=True)
        if status != 200:
    ...
    def download_inovoice_slip(self, **kw):
        ...
        application = request.env['odoocms.application'].sudo().search([('application_no', '=', request.env.user.login)], limit=1)
        if application:
            # report_ref = 'odoocms_admission_portal.action_report_admission_invoices'
            report_ref = 'odoocms_admission.action_report_application_fee_challan'
        if application.fee_voucher_download_date == False or application.fee_voucher_download_date == '' and application.fee_voucher_state != 'verify':
            if application.fee_voucher_state != 'verify' and application.fee_voucher_state in ['no', 'download']:
                application.fee_voucher_state = 'download'
                application.fee_voucher_download_date = datetime.strptime(datetime.now().strftime('%Y-%m-%d'), '%Y-%m-%d').date()
                application.voucher_issued_date = datetime.strptime(datetime.now().strftime('%Y-%m-%d'), '%Y-%m-%d').date()        
        return self._show_report_admit(model=application, report_type=report_type, report_ref=report_ref, download="download")
    ...
    def download_offer_letter(self, **kw):
        report_type = "pdf"
        application = request.env['odoocms.application'].sudo().search([('application_no', '=', request.env.user.login)], limit=1)
        offer_letter = request.env['ucp.offer.letter'].sudo().search([('applicant_id', '=', application.id)], limit=1)
