...
class OdooCMSAdmissionApplication(models.Model):
    ...
    #     if any(name in values for name in ['first_name', 'last_name', 'middle_name']):
    #         student= self.env['odoocms.student'].sudo().search([('admission_no','=',self.application_no)])
    #         if student:
    #             if 'first_name' in values:
    #                 student.first_name = values.get('first_name')
    #             # if 'middle_name' in values:
    #             #     student.middle_name = values.get('middle_name')
    #             if 'last_name' in values:
    #                 student.last_name = values.get('last_name')
    #         name = values.get('first_name') or self.first_name
    #         if values.get('middle_name') or self.middle_name:
    #             middle_name = values.get('middle_name') if values.get('middle_name') else self.middle_name
    #             name = name + ' ' + middle_name
    #         if values.get('last_name') or self.last_name:
    #             last_name = values.get('last_name') if values.get('last_name') else self.last_name
    #             name = name + ' ' + last_name
    #         self.name = name
    #         self.user_id.partner_id.name =name
    #     return super(OdooCMSAdmissionApplication, self).write(values)
    def write(self, values):
        keys_to_capitalize = [
            'first_name', 'middle_name', 'last_name', 'father_name', 'mother_name',
            'street', 'street2', 'per_street', 'per_street2',
            'guardian_name', 'guardian_address', 'fee_payer_name'
        ]
        for key in keys_to_capitalize:
            if key in values and isinstance(values[key], str):
                values[key] = values[key].upper() if values[key] else ""
    
        for rec in self:
            if any(k in values for k in ['first_name', 'middle_name', 'last_name']):
                student = self.env['odoocms.student'].sudo().search([
                    ('admission_no', '=', rec.application_no)
                ])
                if student:
                    if 'first_name' in values:
                ...
                fname = values.get('first_name', rec.first_name)
                mname = values.get('middle_name', rec.middle_name)
                lname = values.get('last_name', rec.last_name)
    
                name = " ".join(filter(None, [fname, mname, lname]))
                values['name'] = name
        ...
        for rec in self:
            if rec.user_id:
                rec.user_id.partner_id.name = rec.name
