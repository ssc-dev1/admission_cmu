...
odoo.define('odoo_admission_ext_a.sms_report', function (require) {
    'use strict';
    ...
    var sms_track = AbstractAction.extend({
        contentTemplate: 'sms_report_summary',
        xmlDependencies: ['/odoocms_reports/static/xml/sms_report_templates.xml'],
        events: {
        ...
        _loadData: function() {
            ...
            this._rpc({
                ...
                self.$('.table_view').html('<div class="alert alert-danger d-flex justify-content-center align-items-center text-center">Failed to load report data.</div>');
                self.report_data = null;
                self._toggleDownloadIcon();
                framework.unblockUI();
            });
        ...
        render_report: function(data) {
            ...
            var template = (data[0] && data[0].length > 0) ? 'sms_report_view' : 'sms_report_view_no_data';
    ...
    core.action_registry.add('sms_report', sms_track);
