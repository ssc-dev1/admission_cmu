...
class ProgramFinancialReportWiz(models.TransientModel):
    _inherit = 'program.wise.fin.summary.wiz'

    main_challan = fields.Boolean("Main Challan", default=False)
    second_challan = fields.Boolean("2nd Challan", default=False)
    admission_challan = fields.Boolean("New Admission", default=False)
    admission_2nd_challan = fields.Boolean("Admission 2nd Challan", default=False)
    add_drop_challan = fields.Boolean("Add Drop", default=False)
    prospectus_challan = fields.Boolean("Prospectus Challan", default=False)
    hostel_challan = fields.Boolean("Hostel Challan", default=False)
    misc_challan = fields.Boolean("Misc Challan", default=False)

    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env.company, required=False)
    institute_ids = fields.Many2many('odoocms.institute', 'program_wise_fin_sum_institute_rel1', 'wiz_id', 'institute_id', 'Faculties')
    program_ids = fields.Many2many('odoocms.program', 'program_wise_fin_sum_program_rel1', 'wiz_id', 'program_id', 'Programs')
    term_ids = fields.Many2many('odoocms.academic.term', 'program_wise_fin_sum_term_rel1', 'wiz_id', 'term_id', 'Terms')
